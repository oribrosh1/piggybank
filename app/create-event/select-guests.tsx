...
import { ArrowLeft, Plus, X, Users, Search, UserPlus, Sparkles, ChevronRight, RefreshCw } from "lucide-react-native";
import { useState, useRef, useEffect } from "react";
import { routes } from "../../types/routes";
import { Guest, EventType, EventCategory, CreateEventData } from "../../types/events";
...
export default function SelectGuestsScreen() {
  ...
  const loadContacts = async () => {
    ...
    try {
      ...
      // Transform contacts to Guest format with default 'added' status
  ...
  const filteredContacts = phoneContacts.filter(
    (c) =>
      ...
      !guests.find((g) => g.id === c.id)
  );

  const addGuest = (contact: Guest) => {
    setGuests([...guests, contact]);
  };
  ...
  const handleContinue = async (skipGuests: boolean = false) => {
    setIsCreatingEvent(true);

    try {
      // Build the event data from params
      const eventData: CreateEventData = {
        ...
        formData: {
          ...
          partyType: params.partyType as string | undefined,
          otherPartyType: params.otherPartyType as string | undefined,
          attireType: params.attireType as string | undefined,
          footwearType: params.footwearType as string | undefined,
          theme: params.theme as string | undefined,
          parking: params.parking as string | undefined,
          kosherType: params.kosherType as string | undefined,
          mealType: params.mealType as string | undefined,
          vegetarianType: params.vegetarianType as string | undefined,
          date: params.date as string || "",
          time: params.time as string || "",
          address1: params.address1 as string || "",
          address2: params.address2 as string || "",
        },
