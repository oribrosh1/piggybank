...
export default function EventDetailsScreen() {
  ...
  const [showEventDetails, setShowEventDetails] = useState(true); // Toggle for event details section
  ...
  useEffect(() => {
    // Animate progress bar to 40% (step 2 of 5)
  ...
  const validateForm = () => {
    ...
    if (eventType === "birthday" && !formData.age.trim())
  ...
  const handleContinue = () => {
    if (validateForm()) {
      router.push({
        ...
        params: { eventType: eventType as "birthday" | "bar-mitzvah" | "bat-mitzvah" | "wedding", ...formData },
      });
    }
  };
  ...
  // For birthdays, always treat as party
