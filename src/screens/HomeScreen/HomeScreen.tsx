...
import { useHomeScreen, type StepConfig } from "./useHomeScreen";
...
export default function HomeScreen() {
  ...
  const renderStepCard = (config: StepConfig, currentStep?: number, _isLast?: boolean) => {
    ...
    const renderEventCard = (event: EventSummary) => {
        ...
        return (
            <TouchableOpacity
                key={event.id}
                onPress={() => goToEventDashboard(event.id)}
                activeOpacity={0.8}
                style={{
                    backgroundColor: '#FFFFFF',
                    borderRadius: 20,
