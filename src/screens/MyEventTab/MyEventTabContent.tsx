...
import { useMyEventTab } from "./useMyEventTab";

const MY_EVENT_PADDING_TOP = 12;
const MY_EVENT_PADDING_H = spacing[5];

export default function MyEventTabContent() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { eventId, firstName, loading, reload } = useMyEventTab();
