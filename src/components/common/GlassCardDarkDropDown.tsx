...
export type GlassCardDarkDropDownProps = {
  children: ReactNode;
  /** Merged onto outer shadow wrapper (e.g. `marginBottom`, `flex`) */
  style?: StyleProp<ViewStyle>;
  /** Merged onto the bordered shell inside the outer wrapper (e.g. `overflow: "visible"` for dropdowns) */
 ...
 */
export function GlassCardDarkDropDown({
  children,
  style,
  shellStyle,
  contentStyle,
...
}: GlassCardDarkDropDownProps) {
  const shellBase: ViewStyle = {
    borderRadius,
    overflow: "hidden",
    borderWidth: 1,
    borderColor,
  ...
  return (
    <View style={[styles.outer, { borderRadius }, style]}>
      <View style={[shellBase, shellStyle]}>
        {Platform.OS === "ios" ? (
